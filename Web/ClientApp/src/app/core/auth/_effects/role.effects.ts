...
// Services
import { RolesAndPermissionService } from '../../../core/_services';
// State
...
export class RoleEffects {
    ...
    */
    ...
    createRole$ = this.actions$
        ...
        @Effect()
        loadRolesList$ = this.actions$
            .pipe(
                ofType<RolesListRequested>(RoleActionTypes.RolesListRequested),
                mergeMap(( { payload } ) => {
                    this.store.dispatch(this.showPageLoadingDistpatcher);
                    const requestToServer = this.auth.getRolesList();
                    return forkJoin(requestToServer);
                }),
                map(response => {
                    const result = response[0];
                    this.store.dispatch(this.hidePageLoadingDistpatcher);
                    return new RolesListLoaded({
                        roles: result.data,
                        total: result.length,
                    });
                }),
    ...
    constructor(private actions$: Actions, private auth: RolesAndPermissionService, private store: Store<AppState>) { }
}
